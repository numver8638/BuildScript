 ...
 */
#ifndef BUILDSCRIPT_UTILS_CHARTYPE_H
#define BUILDSCRIPT_UTILS_CHARTYPE_H
...
namespace BuildScript {
     ...
     */
    inline bool IsNotAllowedUnicodeInitially(char32_t ch) {
        // C11 Annex D.2
     ...
     */
    inline bool IsAllowedUnicode(char32_t ch) {
        // C11 Annex D.1
    ...
    /**
     * @brief Check character is out of valid unicode range.
     *        This does not guarantee that given character is not a unassigned unicode character.
     * @param ch A unicode character.
     * @return true if character is in valid range, otherwise false.
     */
    inline bool IsInvalidCharacter(char32_t ch) {
        return (0x10FFFF < ch)                  // Over Unicode range
            || (0xD800 <= ch && ch <= 0xDFFF);  // Between surrogate pair range
    }
     ...
     */
    inline bool IsIdentifier(char32_t ch) {
        return ch == u'_'
            || (u'a' <= ch && ch <= u'z')
            || (u'A' <= ch && ch <= u'Z')
            || (u'0' <= ch && ch <= u'9')
     ...
     */
    inline bool IsBinary(char32_t ch) {
     ...
     */
    inline bool IsOctal(char32_t ch) {
     ...
     */
    inline bool IsDecimal(char32_t ch) {
     ...
     */
    inline bool IsHexadecimal(char32_t ch) {
        return IsDecimal(ch)
            || (u'a' <= ch && ch <= u'f')
            || (u'A' <= ch && ch <= u'F');
    }
...
#endif // BUILDSCRIPT_UTILS_CHARTYPE_H